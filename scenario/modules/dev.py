...
from scenario import dispatcher, DEV_USERS, telethn, OWNER_ID
...
def allow_groups(update: Update, context: CallbackContext):
    args = context.args
    if not args:
        state = "Lockdown is " + "on" if not scenario.ALLOW_CHATS else "off"
...
def leave(update: Update, context: CallbackContext):
    ...
    if args:
        ...
        update.effective_message.reply_text("I'm going to leave {}, press the button below to confirm".format(chat.title), reply_markup=InlineKeyboardMarkup(scenario_leave_bt))
...
def restart(update: Update, context: CallbackContext):
    ...
    process = subprocess.run("pkill python3 && python3 -m scenario", shell=True, check=True)
    process.communicate()
